...
- Current base seen in this workspace: `6.6.2` era, commit lineage rooted at abbreviated hash `d05ee26`.
...
1. **Build-time config (`pavement.py` templates)**
   - Main control is `template_args` in `runestone/common/project_template/pavement.tmpl`.
...
1. `runestone/common/project_template/pavement.tmpl`
...
2. `runestone/common/project_template/_templates/plugin_layouts/sphinx_bootstrap/layout.html`
...
- `npm run build` currently fails due to missing JS dependency resolution and invalid dependency name entry in `package.json` (`"-": "0.0.1"`).
